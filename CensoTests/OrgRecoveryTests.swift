...
class OrgRecoveryTests: XCTestCase {
    ...
    func testSingleChangeRecoveryTx() throws {
        ...
        XCTAssertEqual(
            Data(SHA256.hash(data: try JSONEncoder().encode(orgRecoveryRequestEnvelope.request))).toHexString(),
    ...
    func testMultipleChangesRecoveryTx() throws {
        ...
        let orgRecoveryRequestEnvelope: OrgAdminRecoveryRequestEnvelope = Mock.decodeJsonType(data: multipleChangesRequest.data(using: .utf8)!)
        let recoveryAppSigningRequest = try orgRecoveryRequestEnvelope.request.toRecoveryAppSigningRequest()
        XCTAssertEqual(
    ...
    func testRecoveryAppFlow() throws {
        ...
        let orgAdminRecoverySignaturesRequest = try OrgAdminRecoverySignaturesRequest.fromRecoveryAppSigningResponse(
            myOrgAdminRecoveryRequest: orgRecoveryRequestEnvelope.request,
