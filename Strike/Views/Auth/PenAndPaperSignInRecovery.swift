...
struct PenAndPaperSignInRecovery: View {
    ...
    private func finish() {
        do {
            ...
            authProvider.authenticate(.signature(email: email, privateKey: privateKey)) { error in
                signingIn = false

                if let _ = error {
                    alert = .couldNotSignIn
                } else {
                    try? Keychain.savePrivateKey(privateKey, rootSeed: rootSeed, email: email)
                }
            }
        } catch {
...
struct PenAndPaperSignInRecovery_Previews: PreviewProvider {
    static var previews: some View {
        NavigationView {
            PenAndPaperSignInRecovery(email: "", authProvider: StrikeAuthProvider())
        }
    }
}
#endif
