//
//  CensoApi+AuthenticatedNonces.swift
//  Censo
//
...
extension MoyaProvider where Target == CensoApi.Target {
    func requestWithNonces(accountAddresses: [String], accountAddressesSlot: UInt64, target: @escaping ([CensoApi.Nonce]) -> Target, completion: @escaping (Result<Moya.Response, Error>) -> Void) {
    ...
    private func sendRequestWithNonces(accountAddresses: [String],
                                       ...
                                       retryCount: Int,
                                       target: @escaping ([CensoApi.Nonce]) -> Target,
                                       completion: @escaping (Result<Moya.Response, Error>) -> Void) {
        self.decodableRequest(
            .multipleAccountNonce(CensoApi.GetMultipleAccountsRequest(accountKeys: accountAddresses)),
            completionQueue: nil
        ) { [weak self] (result: Result<CensoApi.GetMultipleAccountsResponse, MoyaError>) in
            switch result {
            case .success(let response):
